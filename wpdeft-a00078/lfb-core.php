...
class lfb_Core {
                ...
                <div class="lfb_lPanelBodyContent">
                    ...
                    <div class="lfb_componentModel lfb_item" data-component="imageButton">
                        ...
                        <div class="lfb_imageButtonContainer">
                            ...
                            <img  class="lfb_imageButtonImg" src="' . esc_url($this->assets_url) . 'img/placeholder.png" data-placement="bottom"  alt="' . esc_html__('Image with button', 'lfb') . '" />
                    ...
                    <div class="lfb_componentModel lfb_item" data-component="layeredImage">
                     ...
                     <div class="lfb_componentModel lfb_item" data-component="richtext">
                        <div class="lfb_componentTitle">' . esc_html__('Rich text', 'lfb') . '</div>
                        <div class="lfb_componentPreview">
            ...
            if ($log->paid == 0) {
                ...
                if (count($formReq) > 0) {
                    ...
                    if ($form->use_razorpay || $form->use_stripe || ($form->use_paypal && $form->paypal_useIpn)) {
                        ...
                        if ($form->use_stripe) {
                            $response .= '<p class="lfb_btnNextContainer lfb_btnNextContainerMargTop"><a href="javascript:" id="lfb_btnPayStripe"  class="btn btn-wide btn-primary">' . $form->last_btn . '</a></p>';
                        }
                        if ($form->use_paypal) {
    ...
    public function showCustomerManagementPage($pageContent) {
        ...
        if ($settings->enableCustomerAccount && get_the_ID() == $settings->customerAccountPageID) {
            $pageContent = '<div id="lfb_bootstraped" class="lfb_bootstraped"><div id="lfb_form" class="lfb_bootstraped lfb_customerAccount">';
    ...
    public function frontend_enqueue_styles($hook = '') {
        ...
        if ($this->formToPayKey != "" || $this->modeManageData || ($settings->enableCustomerAccount && get_the_ID() == $settings->customerAccountPageID)) {
            ...
            wp_register_style($this->_token . '_bootstrap-select', esc_url($this->assets_url) . 'css/bootstrap-select.min.css', array(), $this->_version);
            ...
            wp_register_style($this->_token . '_fontawesome', esc_url($this->assets_url) . 'css/font-awesome.min.css', array(), $this->_version);
            ...
            wp_enqueue_style($this->_token . '_bootstrap-select');
    ...
    public function conditionally_add_scripts_and_styles($posts) {
        ...
        if (!$this->checkedSc) {
            ...
            if ($shortcode_found && count($this->currentForms) > 0 && !is_admin() && !isset($_GET['ct_builder'])) {
                ...
                wp_register_script($this->_token . '_frontend-libs', esc_url($this->assets_url) . 'js/lfb_frontendPackedLibs.min.js', array("jquery-ui-core", "jquery-ui-tooltip", "jquery-ui-slider", "jquery-ui-position", "jquery-ui-datepicker", 'jquery-effects-core'), $this->_version);
                wp_enqueue_script($this->_token . '_frontend-libs');
                wp_register_script('touchpunch', esc_url($this->assets_url) . 'js/jquery.ui.touch-punch.min.js', array($this->_token . '_frontend-libs'), $this->_version);
                wp_enqueue_script('touchpunch');
                wp_register_script($this->_token . '_frontend', esc_url($this->assets_url) . 'js/lfb_form.min.js', array($this->_token . '_frontend-libs', 'touchpunch'), $this->_version);
                        ...
                        if (!in_array($formID, $formsDone)) {
                            ...
                            if ($form) {
                                ...
                                if ($form->useSignature) {
                                    wp_register_script($this->_token . '_signature', esc_url($this->assets_url) . 'js/jquery.signature.min.js', array(), $this->_version);
                                ...
                                if ($form->datepickerLang != "" && is_file($this->assets_dir . '/js/datepickerLocale/bootstrap-datetimepicker.' . $form->datepickerLang . '.js')) {
                                    ...
                                    wp_register_script($this->_token . '_datetimepicker-locale-' . $form->datepickerLang, esc_url($this->assets_url) . 'js/datepickerLocale/bootstrap-datetimepicker.' . $form->datepickerLang . '.js', array(), $this->_version);
    ...
    public function generateItemHtml($dataItem, $form, $stepData, $isFinalStep) {
        ...
        if ($chkDisplay) {
            ...
            if ($dataItem->type == 'row') {
                ...
                $response .= '<div class="lfb_itemBloc ' . $colClass . ' lfb_row row" data-id="' . $dataItem->id . '" data-columns="' . $dataColumns . '" data-itemtype="' . $dataItem->type . '">';
                ...
                if ($dataItem->quantity_enabled) {
                    if (!$dataItem->useDistanceAsQt && $form->qtType == 1) {
                        ...
                        $response .= ' <input class="lfb_qtfield form-control" min="' . $qtMin . '" ' . $qtMax . ' type="number" value="' . $qtMin . '" /> ';
                        ...
                        $response .= '<div class="quantityBtns lfb_sliderQtContainer" data-stepslider="' . $dataItem->sliderStep . '" data-max="' . $dataItem->quantity_max . '" data-min="' . $dataItem->quantity_min . '">
                                                     <div class="lfb_sliderQt"></div>
                                                 </div>';
                        $response .= '<span class="palette-turquoise icon_quantity lfb_hidden">' . $valMin . '</span>';
                    } else {
                ...
                $response .= '<div id="lfb_map_' . $dataItem->id . '" data-mapstyle="' . $dataItem->mapStyle . '" data-mapzoom="' . $dataItem->mapZoom . '" data-itinerary="' . $dataItem->distanceQt . '" data-height="' . $dataItem->maxHeight . '" class="lfb_itemBloc lfb_gmap  ' . $colClass . '" data-id="' . $dataItem->id . '" data-maptype="' . $dataItem->mapType . '" data-address="' . $dataItem->address . '" data-itemtype="' . $dataItem->type . '" data-itemid="' . $dataItem->id . '" data-title="' . $dataItem->title . '"  ' . $useShowConditions . ' ' . $showConditions . ' ' . $showConditionsOperator . '></div>';
            } else if ($dataItem->type == 'richtext') {
                $response .= '<div class="lfb_itemBloc lfb_richtext  ' . $colClass . '" ' . $showInSummary . '  data-id="' . $dataItem->id . '" data-itemtype="' . $dataItem->type . '" data-itemid="' . $dataItem->id . '" data-title="' . $dataItem->title . '"  ' . $useShowConditions . ' ' . $showConditions . ' ' . $showConditionsOperator . '>' . do_shortcode($dataItem->richtext) . '</div>';
    ...
    public function wpt_shortcode($attributes, $content = null) {
        ...
        if ($form_id != "" && $form_id > 0 && !is_array($form_id)) {
            ...
            if (count($formReq) > 0) {
                ...
                $response .= '<a id="lfb_close_btn" href="javascript:"><span class="fui-cross"></span></a>
                <div id="lfb_panel">
                ...
                foreach ($steps as $stepData) {
                    if ($stepData->formID == $form->id) {
                        ...
                        if (!empty($dataContent) && isset($dataContent->start)) {
                            ...
                            $response .= '	<div class="lfb_genContent container-fluid ' . $contentNoDes . '" >';
                            ...
                            $shineCanvas = '';
                            $response .= '<a href="javascript:" id="lfb_btnNext_' . $stepData->id . '" class="btn btn-wide btn-primary lfb_btn-next ' . $hideNtxStepBtn . '">' . $nextStepIcon . $form->btn_step . '</a>';
                ...
                $response .= '<h3 id="lfb_finalPrice" class="' . $dispFinalPrice . '"><span></span>' . $subTxt . '</h3>';
                ...
                if ($form->useCoupons) {
                    $response .= '<div id="lfb_couponContainer" class="form-group">'
                            ...
                            . '<a href="javascript:" id="lfb_couponBtn" class="btn btn-primary"><span class="glyphicon glyphicon-check"></span></a>'
                            . '</div>';
                }
                ...
                if ($form->use_stripe && $form->paymentType != "email") {
                    ...
                    $response .= '<p  class="lfb_btnNextContainerStripe lfb_stripeMargTop">';
    ...
    public function frontend_enqueue_scripts($hook = '') {
        ...
        if (isset($post->post_title) && get_the_ID() == $settings->previewPageID) {
            ...
            if (isset($_GET['lfb_editForm'])) {
                ...
                wp_register_script($this->_token . '_visualFrontend', esc_url($this->assets_url) . 'js/lfb_visualFrontend.min.js', array('jquery', 'jquery-ui-draggable', 'jquery-ui-droppable', 'jquery-ui-resizable', 'jquery-ui-sortable'), $this->_version);
                wp_enqueue_script($this->_token . '_visualFrontend');

                wp_localize_script($this->_token . '_visualFrontend', 'lfb_data', array(
                    'homeUrl' => get_site_url(),
                    'ajaxurl' => admin_url('admin-ajax.php'),
        ...
        if (($settings->enableCustomerAccount && $settings->customerAccountPageID > 0 && get_the_ID() == $settings->customerAccountPageID)) {
            ...
            global $wpdb;
            wp_register_script($this->_token . '_frontend-libs', esc_url($this->assets_url) . 'js/lfb_frontendPackedLibs.min.js', array("jquery-ui-core", "jquery-ui-tooltip", "jquery-ui-slider", "jquery-ui-position", "jquery-ui-datepicker"), $this->_version);
            wp_enqueue_script($this->_token . '_frontend-libs');
            wp_register_script($this->_token . '_accountManagement', esc_url($this->assets_url) . 'js/lfb_accountManagement.min.js', array('jquery'), $this->_version);
            ...
            wp_localize_script($this->_token . '_accountManagement', 'lfb_dataCust', array(
                'homeUrl' => get_site_url(),
                'ajaxurl' => admin_url('admin-ajax.php'),
        ...
        if ($this->modeManageData) {
            ...
            wp_localize_script($this->_token . '_manageDatas', 'lfb_dataMan', array(
                'homeUrl' => get_site_url(),
                'ajaxurl' => admin_url('admin-ajax.php'),
                'txtCustomersDataDownloadLink' => $settings->txtCustomersDataDownloadLink,
                'txtCustomersDataDeleteLink' => $settings->txtCustomersDataDeleteLink,
        ...
        if ($this->formToPayKey != "") {
            ...
            if (count($rows) > 0) {
                ...
                if (count($logs) > 0) {
                    ...
                    wp_register_script($this->_token . '_bootstrap', esc_url($this->assets_url) . 'js/bootstrap.min.js', array('jquery', "jquery-ui-core"), $this->_version);
                    wp_enqueue_script($this->_token . '_bootstrap');
                    wp_register_script($this->_token . '_payForm', esc_url($this->assets_url) . 'js/lfb_payForm.min.js', array('jquery'), $this->_version);
                    wp_enqueue_script($this->_token . '_payForm');
                    wp_localize_script($this->_token . '_payForm', 'lfb_dataPay', array(
                        'homeUrl' => get_site_url(),
    ...
    private function lfb_generatePdfCustomer($order, $form, $fromCustomer) {
        ...
        if ($fromCustomer) {
            ...
            while (($lastPos = strpos($order->contentUser, '<span class="lfb_value">', $lastPos)) !== false) {
                ...
                $toReplaceDefault[] = '<span class="lfb_value">' . $value . '</span>';
                $toReplaceBy[] = '<span class="lfb_value">shak' . $this->stringDecode($value, $settings->encryptDB) . '</span>';
            }
            foreach ($toReplaceBy as $key => $value) {
    ...
    private function verifyCaptchaThen($form, $formID,
            ...
            $useRtl,
            $fieldsLast,
            $gravity, $signature, $totalText, $vatPrice, $vatAmount, $vatLabel, $callBack) {
        if ($form->useCaptcha && $form->recaptcha3KeySecret != '') {
            ...
            if ($responseKeys["success"]) {
                $callBack($form, $formID,
                        ...
                        $fieldsLast,
                        $gravity,
                        $signature, $totalText, $vatPrice, $vatAmount, $vatLabel);
            } else {
        ...
        if ($showFilesLinks) {
            ...
            while (($lastPos = strpos($projectCustomer, 'class="lfb_file">', $lastPos)) !== false) {
                ...
                if (!in_array($file, $toReplaceDefault)) {
					...
					//wpdeft
                    $imgURL = $this->uploads_url . $formSession . $form->randomSeed . '/' . $file;
                    $toReplaceBy[] = '<img src="'.$imgURL.'" alt="'.$filename.'" width="200" height="150" style="margin-top:20px; !important" />';
    ...
    public function send_email() {
        ...
        $this->verifyCaptchaThen($form,
                ...
                $gravity,
                $signature,
                $totalText, $vatPrice, $vatAmount, $vatLabel) {
            global $wpdb;
            $settings = $this->getSettings();
            if ($discountCode != "") {
                            ...
                            if (isset($value['isFile']) && $value['isFile'] == 'true' && $attribute == 'value') {
                                ...
                                while (($i_lastPos = strpos($newValue, 'class="lfb_file">', $i_lastPos)) !== false) {
									...
									if (!$noSpan) {
										...
										$newValue = '<img src="' . $this->uploads_url . $formSession . $form->randomSeed . '/' . $file . '" width="338" height="192" style="margin-top:20px; !important" >';
										
                                    } else {
                                       $newValue = '<img src="' . $this->uploads_url . $formSession . $form->randomSeed . '/' . $file . '">';
                                    }
                                }
                            }
    ...
    public function cart_save() {
        ...
        if (count($formReq) > 0) {
            ...
            foreach ($products as $product) {
                ...
                if ($form->sendSummaryToWoo) {
                    $summaryTxt = str_replace('[n]', '<br/>', sanitize_text_field($_POST['contentTxt']));
